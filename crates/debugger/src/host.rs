...
mod generated {
    ::wasmtime::component::bindgen!({
        ...
        with: {
            "wasmtime:debugger/debuggee.debuggee": super::Debuggee,
            "wasmtime:debugger/debuggee.event-future": super::EventFuture,
            "wasmtime:debugger/debuggee.frame": super::Frame,
            "wasmtime:debugger/debuggee.instance": ::wasmtime::Instance,
            "wasmtime:debugger/debuggee.module": ::wasmtime::Module,
            "wasmtime:debugger/debuggee.table": ::wasmtime::Table,
            "wasmtime:debugger/debuggee.global": ::wasmtime::Global,
            "wasmtime:debugger/debuggee.memory": ::wasmtime::Memory,
            "wasmtime:debugger/debuggee.wasm-tag": ::wasmtime::Tag,
            "wasmtime:debugger/debuggee.wasm-func": ::wasmtime::Func,
            "wasmtime:debugger/debuggee.wasm-exception": super::WasmException,
            "wasmtime:debugger/debuggee.wasm-value": super::WasmValue,
...
pub use generated::Main as DebuggerComponent;
pub use generated::wasmtime::debugger::debuggee as wit;
...
impl<T: Send + 'static> OpaqueDebugger for crate::Debugger<T> {
    ...
    async fn memory_grow(&mut self, memory: Memory, delta_bytes: u64) -> Result<WitResult<u64>> {
        self.with_store(move |mut store| {
            let page_size = u64::try_from(memory.page_size(&store)).unwrap();
...
impl<T: DebuggerView> wit::HostInstance for DebuggerImpl<T> {
    async fn get_module(
        ...
        let i = self.table().get(&self_)?.clone();
    ...
    async fn get_memory(
        ...
        let instance = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
...
impl<T: DebuggerView> wit::HostMemory for DebuggerImpl<T> {
    async fn size_bytes(&mut self, self_: Resource<Memory>, d: Resource<Debuggee>) -> Result<u64> {
        let memory = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
...
impl<T: DebuggerView> wit::HostGlobal for DebuggerImpl<T> {
    async fn get(
        ...
        let global = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
...
impl<T: DebuggerView> wit::HostTable for DebuggerImpl<T> {
    async fn len(&mut self, self_: Resource<Table>, d: Resource<Debuggee>) -> Result<u64> {
        let table = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
...
impl<T: DebuggerView> wit::HostWasmFunc for DebuggerImpl<T> {
    async fn params(
        ...
        let func = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
...
impl<T: DebuggerView> wit::HostWasmException for DebuggerImpl<T> {
    ...
    async fn make(
        ...
        let tag_val = self.table().get(&tag)?.clone();
...
impl<T: DebuggerView> wit::HostWasmTag for DebuggerImpl<T> {
    async fn params(
        ...
        let tag = self.table().get(&self_)?.clone();
        let d = debugger(self, &d)?;
