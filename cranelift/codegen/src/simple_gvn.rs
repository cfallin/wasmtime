...
pub fn do_simple_gvn(func: &mut Function, domtree: &mut DominatorTree) {
    ...
    let mut visible_values: ScopedHashMap<HashKey, Inst> = ScopedHashMap::with_capacity(16);
        ...
        while let Some(inst) = {
            ...
            // Resolve aliases, particularly aliases we created earlier.
            {
                ...
                for i in 0..pos.func.dfg.inst_args(inst).len() {
                    ...
                    pos.func.dfg.inst_args_mut(inst)[i] = arg;
                }
            }
            ...
            match visible_values.entry(&crate::ctxhash::NullCtx, key) {
                Occupied(entry) => {
