...
use crate::ir::{
    Block, BlockArg, DataFlowGraph, Function, Inst, InstBuilder, InstructionData, Layout, Opcode,
    Type, Value,
};
...
fn create_pre_header(
    ...
    pos.next_inst();
    let args = pre_header_args_value
        .as_slice(pool)
        .iter()
        .map(|value| BlockArg::Value(*value))
        .collect::<Vec<_>>();
    pos.ins().jump(header, &args);
...
fn remove_loop_invariant_instructions(
    ...
    let mut loop_values: FxHashSet<Value> = FxHashSet::default();
...
fn postorder_blocks_loop(
    ...
    let mut grey = FxHashSet::default();
