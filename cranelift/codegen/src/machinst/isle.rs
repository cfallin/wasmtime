...
pub type OptionInst = PackedOption<Inst>;

/// Helper macro to import all definitions used by the common prelude.
#[macro_export]
#[doc(hidden)]
macro_rules! isle_common_prelude_uses {
    () => {
        ...
        use crate::{
            ...
            ir::{
                ...
                immediates::*,
                types::*,
                AtomicRmwOp, Endianness, Inst, InstructionData, MemFlags, Opcode, StackSlot,
...
/// Helper macro to define methods in `prelude.isle` within `impl
/// Context for ...` for each ISLE context. These methods are shared
/// amongst all contexts: both the machine backends, and middle-end
...
macro_rules! isle_common_prelude_methods {
    () => {
        ...
        fn value_slice_empty(&mut self, slice: ValueSlice) -> Option<()> {
            let (list, off) = slice;
            if off >= list.len(&self.dfg().value_lists) {
                Some(())
            } else {
                None
            }
        ...
        fn value_slice_unwrap(&mut self, slice: ValueSlice) -> Option<(Value, ValueSlice)> {
            let (list, off) = slice;
            if let Some(val) = list.get(off, &self.dfg().value_lists) {
                Some((val, (list, off + 1)))
            } else {
                None
            }
        ...
        fn value_slice_len(&mut self, slice: ValueSlice) -> usize {
            let (list, off) = slice;
            list.len(&self.dfg().value_lists) - off
        }

        #[inline]
        fn value_slice_get(&mut self, slice: ValueSlice, idx: usize) -> Value {
            let (list, off) = slice;
            list.get(off + idx, &self.dfg().value_lists).unwrap()
        }
        ...
        fn inst_results(&mut self, inst: Inst) -> ValueSlice {
            (self.dfg().inst_results_list(inst), 0)
        }

        #[inline]
        fn first_result(&mut self, inst: Inst) -> Option<Value> {
            self.dfg().inst_results(inst).first().copied()
        }
        ...
        fn func_ref_data(&mut self, func_ref: FuncRef) -> (SigRef, ExternalName, RelocDistance) {
            let funcdata = &self.dfg().ext_funcs[func_ref];
            (
                funcdata.signature,
                funcdata.name.clone(),
        ...
        #[inline]
        fn inst_to_option_inst(&mut self, inst: Inst) -> Option<OptionInst> {
            Some(PackedOption::from(inst))
        }

        #[inline]
        fn no_inst(&mut self) -> Option<OptionInst> {
            Some(PackedOption::default())
        }

        #[inline]
        fn option_inst_to_inst(&mut self, inst: OptionInst) -> Option<Inst> {
            inst.into()
        }

        #[inline]
        fn option_inst_is_none(&mut self, inst: OptionInst) -> Option<()> {
            if inst.is_none() {
                Some(())
            } else {
                None
...
/// Mid-end-analysis-specific ISLE context implementation.
pub mod analysis {
    use crate::facts::FactTables;
    ...
    pub(crate) struct IsleContext<'a> {
        pub func: &'a Function,
        pub flags: &'a Flags,
...
/// Lowering-specific ISLE context implementation.
pub mod lower {
    use crate::ir::{types, DataFlowGraph, Inst};
    use crate::machinst::{get_output_reg, InsnOutput, LowerCtx};

    use super::InstOutput;

    /// Helper macro to define methods that provide extern constructors
    /// for VCode output specifically. These methods are included in ISLE
    /// contexts for each machine backend's instructino lowering.
    #[macro_export]
    #[doc(hidden)]
    macro_rules! isle_vcode_output_methods {
        () => {
            #[inline]
            ...
            #[inline]
            fn output_builder_push(
                &mut self,
                builder: &InstOutputBuilder,
                regs: ValueRegs,
            ) -> Unit {
    ...
    impl<'a, C, F, I, const N: usize> IsleContext<'a, C, F, I, N>
    where
        C: LowerCtx,
        [(C::I, bool); N]: smallvec::Array,
    {
