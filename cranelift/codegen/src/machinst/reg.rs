...
use regalloc2::{Allocation, Operand, PReg, PRegMask, VReg};
...
impl<'a, F: Fn(VReg) -> VReg> OperandCollector<'a, F> {
    ...
    /// clobber set.
    pub fn finish(self) -> ((u32, u32), PRegMask) {
    ...
    pub fn reg_clobbers(&mut self, regs: PRegMask) {
        self.clobbers.add_all(regs);
    }
}
